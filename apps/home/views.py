...
def load_comunas(request):
    print('=' * 80)
    print('LLAMADA A LOAD_COMUNAS')
    print('REQUEST METHOD:', request.method)
    print('REQUEST.GET:', request.GET)
    print('REQUEST.POST:', request.POST)
    
    region_id = request.GET.get('region_id')
    print(f'REGION_ID RECIBIDO : "{region_id}" (type: {type(region_id)})')
    
    if not region_id or region_id == '':
        print('ERROR: No se recibió region_id o está vacío')
...
def ver_conserjes_view(request):
    print("=" * 80)
    print("ENTRÓ A VER_CONSERJES_VIEW")
    print("REQUEST.GET:", request.GET)
    ...
    try:
        ...
        print("Query construido:", query)
        empleados = Empleados.objects.filter(query)
        print(f"Empleados encontrados: {empleados.count()}")
        
        # Verificar si hay empleados
        for emp in empleados[:5]:  # Solo los primeros 5 para debug
            print(f"Empleado: {emp.rut} - {emp.nombres}")
        ...
        print("!" * 80)
        print("ERROR EN VER_CONSERJES_VIEW:")
        print(f"Tipo de error: {type(e).__name__}")
        print(f"Mensaje: {str(e)}")
        import traceback
        traceback.print_exc()
        print("!" * 80)
        # Re-lanzar el error para que Django lo maneje
        raise
